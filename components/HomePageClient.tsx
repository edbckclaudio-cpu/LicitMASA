...
export default function HomePage() {
  ...
  const loadUserPlanInFlightRef = useRef(false)
  const pendingLoadUserPlanRef = useRef(false)
  const loadUserPlanTimerRef = useRef<number | null>(null)
  ...
  const runDeferredPlanSync = useCallback(async (user: { id: string, email?: string | null }, currentPremium: boolean) => {
    if (!supabase) return
    if (deferredPlanSyncInFlightRef.current) return
    deferredPlanSyncInFlightRef.current = true
    try {
      let premium = currentPremium
      const email = String(user.email || '').toLowerCase()

      // A sincronização do OneSignal permanece, mas fora do caminho crítico da home.
      try {
      ...
      deferredPlanSyncInFlightRef.current = false
    }
  }, [playProductId])
  const loadUserPlan = useCallback(async () => {
    if (!supabase) return
    if (loadUserPlanInFlightRef.current) {
      pendingLoadUserPlanRef.current = true
      return
    }
    loadUserPlanInFlightRef.current = true
    setPlanLoading(true)
    try {
      ...
      window.setTimeout(() => {
        void runDeferredPlanSync({ id: user.id, email: user.email || null }, premium)
      }, initialResultsSettledRef.current ? 1500 : 4000)
    } finally {
      loadUserPlanInFlightRef.current = false
      setPlanLoading(false)
      if (pendingLoadUserPlanRef.current) {
        pendingLoadUserPlanRef.current = false
        window.setTimeout(() => { void loadUserPlan() }, 0)
      }
    }
  }, [runDeferredPlanSync])
  const scheduleLoadUserPlan = useCallback((delayMs: number = 0) => {
    if (typeof window === 'undefined') return
    if (loadUserPlanTimerRef.current) {
      window.clearTimeout(loadUserPlanTimerRef.current)
    }
    loadUserPlanTimerRef.current = window.setTimeout(() => {
      loadUserPlanTimerRef.current = null
      void loadUserPlan()
    }, delayMs)
  }, [loadUserPlan])
  useEffect(() => {
    ...
    const fallbackTimer = window.setTimeout(() => {
      if (!initialResultsSettledRef.current) {
        void loadUserPlan()
      }
    }, 5000)
    return () => {
  ...
  useEffect(() => {
    const sub = supabase?.auth?.onAuthStateChange?.((_event: any, _session: any) => { scheduleLoadUserPlan(300) })
    return () => { try { (sub as any)?.data?.subscription?.unsubscribe?.() } catch {} }
  }, [scheduleLoadUserPlan])
  useEffect(() => {
    const onFocus = () => { scheduleLoadUserPlan(300) }
    const onVis = () => { try { if (document.visibilityState === 'visible') scheduleLoadUserPlan(300) } catch {} }
  ...
  const carregarIniciais = useCallback(async () => {
    ...
    try {
      ...
      if (!initialResultsSettledRef.current) {
        initialResultsSettledRef.current = true
        scheduleLoadUserPlan(250)
      }
      requestInFlightRef.current = false
      setLoading(false)
    }
  }, [inicio, hoje, pagina, tamanhoPagina, scheduleLoadUserPlan, sortResultados])
